...
dateMate.randomise = function () {
  if (dateMate.timeToPlot) {
    ...
    if (this.colOneUnfixed) {
      this.getDataForFruitMachine(this.choice1, 1);
    }
    if (this.colTwoUnfixed) {
      this.getDataForFruitMachine(this.choice2, 2);
    }
    if (this.colThreeUnfixed) {
      this.getDataForFruitMachine(this.choice3, 3);
    }
  }
};
...
dateMate.plotIt = function () {
  ...
  if (dateMate.secondStop) {
    dateMate.showLocation(dateMate.secondStop);
  }
  if (dateMate.thirdStop) {
    dateMate.showLocation(dateMate.thirdStop);
  }
};
...
dateMate.showLocation = function (venue) {
  if (venue.restaurant) {
    // let latLngArray = [];
    var latlng = new google.maps.LatLng(venue.restaurant.lat, venue.restaurant.lng);
    ...
    var _latlng = new google.maps.LatLng(venue.bar.lat, venue.bar.lng);
    dateMate.latLngArray.push(_latlng);
    var _icon = {
    ...
    dateMate.markers.push(_marker);
    dateMate.addInfoWindow(venue, _marker);
  }
  dateMate.shownLocations++;
  var bounds = new google.maps.LatLngBounds();
  for (var i = 0; i < dateMate.markers.length; i++) {
...
dateMate.showRoute = function (value) {
  ...
  var dateRoute = new google.maps.DirectionsService();
  var routeDisplayer = new google.maps.DirectionsRenderer({
...
dateMate.handleForm = function () {
  ...
  var url = '' + dateMate.apiUrl + $(this).attr('action');
  var method = $(this).attr('method');
  var data = $(this).serialize();

  return dateMate.ajaxRequest(url, method, data, function (data) {
