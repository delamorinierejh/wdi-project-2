...
dateMate.getInfoForSideBar = function(){
  if (dateMate.choice1 === 'Dinner'){
    ...
    const website = dateMate.firstStop.restaurant.website || '';
    ...
    const website = dateMate.firstStop.bar.website || '';
    dateMate.infoForVenues += `
      <div class="side-box">
      <h2>${dateMate.choice1}</h2>
  ...
  if (dateMate.secondStop){
    if (dateMate.choice2 === 'Dinner'){
      ...
      const website = dateMate.secondStop.restaurant.website || '';
      dateMate.infoForVenues += `
        <div class="side-box">
      ...
      const website = dateMate.secondStop.bar.website || '';
      dateMate.infoForVenues += `
        <div class="side-box">
        <h2>${dateMate.choice2}</h2>
  ...
  if (dateMate.thirdStop){
    if (dateMate.choice3 === 'Dinner'){
      ...
      const website = dateMate.thirdStop.restaurant.website || '';
      dateMate.infoForVenues += `
        <div class="side-box">
      ...
      const website = dateMate.thirdStop.bar.website || '';
      dateMate.infoForVenues += `
        <div class="side-box">
        <h2>${dateMate.choice3}</h2>
...
dateMate.showLocation = function(venue){
  if(venue.restaurant){
                      // let latLngArray = [];
    const latlng = new google.maps.LatLng(venue.restaurant.lat, venue.restaurant.lng);
    ...
    const latlng = new google.maps.LatLng(venue.bar.lat, venue.bar.lng);
    dateMate.latLngArray.push(latlng);
    const icon = {
...
dateMate.showRoute = function(value){
  ...
  const dateRoute = new google.maps.DirectionsService();
  const routeDisplayer = new google.maps.DirectionsRenderer({
...
dateMate.handleForm = function(){
  ...
  const url    = `${dateMate.apiUrl}${$(this).attr('action')}`;
